...
TEST(FlexValueTest, MapWithFields) {
  Value m = make_map({{"width", Value{int32_t{1920}}},
...
TEST(FlexValueTest, NestedMapInArray) {
  Value inner =
      make_map({{"x", Value{int32_t{10}}}, {"y", Value{int32_t{20}}}});
  Value arr = make_array();
...
TEST(FlexValueTest, EqualityMaps) {
  Value m1 = make_map({{"a", Value{int32_t{1}}}, {"b", Value{int32_t{2}}}});
  Value m2 = make_map({{"a", Value{int32_t{1}}}, {"b", Value{int32_t{2}}}});
  Value m3 = make_map({{"a", Value{int32_t{1}}}, {"b", Value{int32_t{3}}}});
...
TEST(FlexValueTest, CopyValue) {
  Value orig = make_map({{"key", Value{std::string{"value"}}}});
  Value copy = orig;
...
TEST(FlexValueTest, ToStringMap) {
  Value m = make_map({{"x", Value{int32_t{10}}}});
...
TEST(FlexValueTest, DVB_PAT_Scenario) {
  Value pat = make_map({{"table_id", Value{uint32_t{0x00}}},
                        {"pid", Value{uint32_t{0x0000}}},
                        {"version_number", Value{uint32_t{1}}},
                        {"transport_stream_id", Value{uint32_t{1234}}}});

  Value programs = make_array();
  array_push(programs, make_map({{"program_number", Value{uint32_t{1}}},
                                 {"pid", Value{uint32_t{256}}}}));
  array_push(programs, make_map({{"program_number", Value{uint32_t{2}}},
...
TEST(FlexValueCOW, ValueCopySharingMemory) {
  Value v1 = make_map({{"key", Value{std::string{"data"}}}});
  Value v2 = v1;
...
TEST(FlexValueBuilder, Array) {
  ...
  Value v = builder.push(int32_t{1}).push(int32_t{2}).push(int32_t{3}).build();
...
TEST(FlexValueTraversal, SimpleTraverse) {
  Value root = make_map({{"a", Value{int32_t{1}}},
...
TEST(FlexValueFind, FindByType) {
  Value root = make_map({{"int1", Value{int32_t{10}}},
                         {"str1", Value{std::string{"text"}}},
                         {"int2", Value{int32_t{20}}},
                         {"arr", make_array()}});

  auto results = find_all(root, [](const Value &v) { return v.is<int32_t>(); });
...
TEST(FlexValueFind, FindByValue) {
  Value arr = make_array();
  array_push(arr, make_map({{"id", Value{int32_t{1}}},
                            {"name", Value{std::string{"Alice"}}}}));
  array_push(arr, make_map({{"id", Value{int32_t{2}}},
                            {"name", Value{std::string{"Bob"}}}}));
  array_push(arr, make_map({{"id", Value{int32_t{3}}},
                            {"name", Value{std::string{"Alice"}}}}));
...
TEST(FlexValueMapIndex, HasKeyCheck) {
  Value m = make_map({{"key1", Value{int32_t{10}}}});
...
TEST(FlexValueSchema, ValidateSimpleMap) {
  ...
  Value valid = make_map(
      {{"name", Value{std::string{"John"}}}, {"age", Value{int32_t{30}}}});
...
TEST(FlexValueIntrospection, MapKeys) {
  Value m = make_map({{"alpha", Value{int32_t{1}}},
...
TEST(FlexValueIntrospection, TypeName) {
  ...
  EXPECT_EQ(ValueIntrospector::type_name(Value{std::string{"test"}}), "string");
...
TEST(FlexValueComplex, DVB_PMT_WithBuilderAndValidation) {
  ...
  auto numeric_values = find_all(
      pmt, [](const Value &v) { return v.is<uint32_t>() || v.is<int32_t>(); });
...
TEST(FlexValueComplex, Deep_Nested_Query) {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    for (int j = 0; j < 10; ++j) {
      Value item = make_map({{"id", Value{int32_t{i * 10 + j}}},
                             {"active", Value{j % 2 == 0}},
                             {"tags", make_array()}});
  ...
  auto active = find_all(
      root, [](const Value &v) { return v.is<bool>() && v.as<bool>(); });
...
TEST(FlexValueExceptions, InvalidArgumentException) {
  ...
  try {
    ...
    EXPECT_TRUE(std::string(e.what()).find("denominator") != std::string::npos);
  }
}
